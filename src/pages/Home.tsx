...
const Home = (): JSX.Element => {
	...
	useEffect(() => {
		(async () => {
			await refreshJoke();
			if (!localStorage.getItem('savedJokes')) localStorage.setItem('savedJokes', '');
			else setSaved(localStorage.getItem('savedJokes') || '');
		})();
	}, []);
	...
	const getJoke = async (): Promise<string> => {
		...
		if (res.data.type === 'single') newJoke = res.data.joke;
		else newJoke = res.data.setup + ' ' + res.data.delivery;
		return newJoke;
	};
	...
	return (
		<>
			<Center
				...
				h="100vh"
			>
				<Text fontWeight="bold" fontSize="5xl" top="20%" position="fixed">
					We have a joke for you !
				</Text>
				<Box w="50%" bg="blue.500" borderRadius="25px" color="white" fontSize="3xl" padding="2%" margin="3%">
